...
std::filesystem::path make_unique_manual_duplicate_output_path(const std::filesystem::path &original_path) {
    ...
    for (int copy_index = 1; copy_index < 1000; ++copy_index) {
        ...
        const std::filesystem::path candidate_path = parent_path.empty()
            ? std::filesystem::path(file_name)
            : (parent_path / file_name).lexically_normal();
        if (normalize_path_key(candidate_path) == original_key || output_path_exists(candidate_path)) {
            continue;
        }
...
DuplicateEncodeEntryResult duplicate_encode_entry(const DuplicateEncodeEntryRequest &request) {
    ...
    if (request.original.output_path_manual_override && !original_output_path.empty()) {
        ...
        duplicate.output_path = path_to_qstring(make_unique_manual_duplicate_output_path(original_output_path));
    ...
    if (!reservations.empty()) {
        duplicate.output_path = path_to_qstring(reservations.front().result.output_path);
        return DuplicateEncodeEntryResult{
            .duplicate = std::move(duplicate),
