...
if [[ "${build_app}" == "ON" ]]; then
  ...
  cmake --build "${build_dir}" --target utsure_app_windows_taskbar_progress_tests --parallel
  cmake --build "${build_dir}" --target utsure_app_queue_terminal_notification_tests --parallel
...
cmake --build "${build_dir}" --target utsure_core_ffmpeg_filter_hardsub_backend_tests --parallel
