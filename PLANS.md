...
- The current M24 slice also permits one narrow user-requested benchmark automation follow-up, limited to exporting machine-readable per-stage timing from the real streaming encode path, adding one noninteractive benchmark runner, and wiring one additional benchmark job into the primary Windows workflow so it can run manually or automatically without changing normal desktop behavior or codec defaults.
...
Current slice status:
  ...
  * Completed: added a benchmark-only export/report path around the existing streaming transcode metrics and wired it into the primary Windows workflow as a dedicated benchmark job, so CI can capture deterministic per-stage timing, raw logs, and output artifacts for `x264` versus `x265` runs without forking the encode pipeline or splitting that coverage into a second workflow entry point.
...
Finish M24 by running the real subtitle-enabled before/after encode comparison and confirming output/timing correctness in CI or another approved runtime environment.
