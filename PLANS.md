...
Current slice status:
  ...
  * Completed: fixed the FontCollector failure diagnostics follow-up by preserving failed tool logs under the OS temp diagnostics area and including the preserved `fontcollector.log` path in the blocking error hint, so users can inspect exit-code failures after preflight cleanup.
