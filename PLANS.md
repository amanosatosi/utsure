...
- `libassmod` is currently treated as a pinned source dependency built into an isolated prefix, currently pointed at upstream commit `3f5640045bbd737661f3858480ec948fece929cb`, because it installs as a `libass`-compatible package name rather than a uniquely named `libassmod` package.
...
Current slice status:
  ...
  * Completed: traced the real Kanteishi episode 06 failure to an always-active gradient watermark that keeps libassmod in RGBA output mode at the `0:20:04.76` BorderStyle=4 box, then moved the CI/default libassmod pin forward to upstream `3f5640045bbd737661f3858480ec948fece929cb` so utsure uses the same unpatched RGBA output-management fix line instead of a local source patch, with a focused persistent-RGBA-plus-BS4 fixture wired into both the libassmod-only reproducer and utsure render tests.
