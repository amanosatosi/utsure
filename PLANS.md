...
Current slice status:
  ...
  * In progress: refine the subtitle AddressSanitizer coverage so GitHub Actions reports each of the four bitmap/composition isolation modes separately, repeats the stress workload long enough to approximate the prior late-session crash window, and summarizes the exact failing mode or common factor without requiring a local build; remote validation is routed through the existing workflow on `main`, `pull_request`, or temporary `codex/*` validation branches, the workflow keeps the four-mode matrix running even if the compositor-only ASan precheck fails, the standalone matrix step now restores the same dependency PATH/runtime environment as the build step so the stress binaries actually execute instead of failing immediately in a fresh shell, and the stress harness/matrix summary now emit millisecond timing plus structured `failure_stage` and `failure_reason` metadata because cached FFmpeg builds make second-level wall-clock timings too coarse to diagnose early failures. The earlier shared startup failure root cause was the CTest stress registration using `cmake -E env` with a bare `utsure_core_subtitle_burn_in_tests` token instead of `$<TARGET_FILE:...>`, which could fail before the stress binary started; the current hardening pass therefore adds an explicit representative startup check plus stage-tagged asset/dependency/source/session preflight before full matrix attribution resumes, the harness now treats discovery as a plain test-existence check via `ctest -N --no-tests=error` instead of depending on human-readable `Test command:` output, and representative execution now logs the exact escaped `ctest` invocation plus raw stdout/stderr while requiring the test's own early startup marker before the run is considered launched.
  * In progress: the Windows sanitizer CI lane itself was then found to be misconfigured because MSYS2 only ships Clang/LLD sanitizer runtimes in `clang*` environments, while the job was still running under `UCRT64` with GCC and hard-coded `/ucrt64` lookup paths. The current hardening pass therefore also moves only the `subtitle-asan` job to `CLANG64`, derives the MSYS2 prefix from the active environment in the shared CI scripts, and adds an early CMake configure-time guard so unsupported Windows ASan setups fail with a specific message instead of a late `cannot find -lasan` linker error.
  * In progress: after moving the sanitizer lane to `CLANG64`, the source-built FFmpeg/FFMS2 dependency scripts were still leaving compiler selection implicit, so FFmpeg `configure` could still default to `gcc` and fail its compiler probe even inside the clang environment. The current CI hardening therefore also pins the active dependency toolchain explicitly (`clang`/`clang++`/LLVM binutils in `clang*` environments, GCC/binutils elsewhere) and narrows the FFmpeg cache restore keys so the sanitizer lane no longer rehydrates the old `ucrt64-gcc` cache as a fallback.
  * In progress: the follow-on FFMS2 failure under `CLANG64` is not a ref change; the pinned commit stays on `25cef14386fcaaa58ee547065deee8f6e82c56a2`. The current evidence points instead to using the generic MSYS `libtool` package in the clang job, which does not carry the MinGW/llvm-mingw fixes that MSYS2 ships in the target-specific `mingw-w64-clang-x86_64-libtool` package. The current CI hardening therefore switches the sanitizer lane onto the clang-targeted libtool package so FFMS2's autotools/libtool link step can handle LLVM runtime objects correctly.
  * In progress: even with the clang-targeted libtool package, the source-built FFMS2 step remains blocked by bundled autotools/libtool incompatibility with llvm-mingw when linking C++ under `CLANG64`. Because the subtitle ASan lane only needs FFMS2 available for linking the wider test build and does not exercise FFMS2 behavior in the subtitle stress workload itself, the current CI hardening now consumes MSYS2's prebuilt `mingw-w64-clang-x86_64-ffms2` package only in the `subtitle-asan` job, via an explicit `UTSURE_FFMS2_USE_SYSTEM_PACKAGE=ON` escape hatch, while keeping the pinned source-build FFMS2 path unchanged for the main Windows build job.
  * In progress: after introducing the prebuilt `clang64` FFMS2 package in the sanitizer lane, the package-set fallback exposed a system `libass.pc`, and the staged libassmod install was not guaranteeing a same-prefix `libass.pc` in the deterministic `${prefix}/lib/pkgconfig` location that both the dependency audit and CMake dependency checks expect. The current hardening therefore normalizes libassmod's installed pkg-config metadata after `meson install`: if libassmod already produced a `libass.pc`, it is copied into `${prefix}/lib/pkgconfig`; otherwise a small libass-compatible shim is generated there so the staged libassmod prefix remains authoritative over any system `libass` package.
  * In progress: the remaining libassmod audit failure under the sanitizer lane was caused by shared CI scripts still prepending `${ffms2_prefix}/lib/pkgconfig` ahead of the staged libassmod prefix. With `UTSURE_FFMS2_PREFIX=/clang64`, that meant `/clang64/lib/pkgconfig/libass.pc` shadowed `.deps/libassmod/prefix/lib/pkgconfig/libass.pc`. The current hardening therefore reorders shared `PKG_CONFIG_PATH` setup to put the staged libassmod pkg-config directory first, followed by FFmpeg and FFMS2, so the subtitle renderer dependency remains pinned to the repo-built libassmod even when FFMS2 comes from the system `clang64` package set.
  * Completed: fixed the subtitle stress matrix shell-parse failure in the MSYS2 CI lane by replacing the invalid line-61 `case` pattern in `escape_ctest_regex`, keeping the harness explicitly Bash-only, normalizing the script to LF/no-BOM text, and changing the workflow to run `bash -n` before `bash ./scripts/ci/windows-msys2-run-subtitle-stress-matrix.sh`.
  * Completed: fixed the follow-on representative/matrix test-selection bug by separating exact CTest regex construction from shell-formatted logging, so discovery and execution both pass the same raw anchored regex string to `ctest -R`, shell-escaped rendering stays log-only, and discovery now requires a real nonzero CTest match count instead of trusting `ctest -N` exit status alone.
  * Completed: fixed the next subtitle ASan preflight gap by separating sanitizer configure/build from runtime coverage, adding an explicit `ctest -N --test-dir build -C Debug` discovery step that prints the full CTest list plus `burn_in`/`stress`/`copied_serialized`/`h264` matches, proving the representative stress test exists before compositor coverage or the isolation matrix runs, and failing early with a build-tree/CTest-registration diagnostic when source registration exists but discovery is empty.
