...
- The current M24 slice also permits one narrow user-requested benchmark automation follow-up, limited to exporting machine-readable per-stage timing from the real streaming encode path, adding one noninteractive benchmark runner, and wiring one Windows benchmark artifact workflow that can run manually or automatically without changing normal desktop behavior or codec defaults.
