...
- The current subtitle-rendering behavior mirrors Aegisub's libassmod provider: render through `ass_render_frame_auto()`, consume `ASS_ImageRGBA` only when libassmod marks the frame as requiring RGBA output, and otherwise consume the legacy `ASS_Image` list so upstream libassmod behavior remains unchanged.
...
- The current M24 slice also permits one narrow user-requested intro/outro compatibility follow-up, limited to keeping the main source authoritative while normalizing common intro/outro cadence, raster, sample-aspect-ratio, and audio sample-rate/channel-count mismatches in-core and in the streaming path, while keeping missing main-audio targets and invalid decoded timing/layout cases fatal.
...
- The current M24 slice also permits one narrow user-requested preview trim follow-up, limited to making the FFMS2/Qt preview path honor the selected main-source trim window exactly by clipping preview-side decoded frame/audio windows, rebasing preview playback state on trim edits, and keeping export behavior unchanged.
...
- The current M24 slice also permits one narrow user-requested desktop-shell encode-progress follow-up, limited to surfacing the existing frame-driven encode percentage in the Status text and existing rotating start-button indicator without changing broader layout, queue behavior, or core encode policy.
...
- The M25 implementation assumes `thumbnail.*` matching is case-insensitive on the file stem and supports common FFmpeg-readable still formats (`.png`, `.jpg`, `.jpeg`, `.webp`, `.bmp`, `.tif`, `.tiff`); manually chosen thumbnail images may differ from final output dimensions when they match the output aspect ratio within encoder-safe resize rounding tolerance and can be normalized safely. Thumbnail upscaling is not enabled, so smaller manual images are rejected with an explicit diagnostic instead of being silently enlarged.
...
- Scope update: the adapter now uses libassmod's automatic render-result API so gradient-capable frames use `ASS_ImageRGBA` while normal frames, including plain BorderStyle=4 boxes, can follow the upstream legacy `ASS_Image` path.
...
Current slice status:
  ...
  * Completed: wired the Windows portable release job to freeze the pinned `FontCollector` package into a standalone `fontcollector.exe`, package it under `tools/fontcollector`, and smoke-test that executable from the extracted zip before uploading the artifact.
  ...
  * Completed: removed the separate subtitle-asan GitHub Actions job to keep CI simpler while leaving `UTSURE_ENABLE_ADDRESS_SANITIZER` and the subtitle compositor/burn-in test targets available for future manual or scheduled sanitizer runs.
  ...
  * Completed: tuned the desktop-shell Qt scrollbar styling so the thumb/handle now uses the existing gold accent instead of the black fallback, while keeping the track and surrounding shell styling unchanged.
  ...
  * Completed: fixed the preview-only trim follow-up by threading trim bounds through the preview frame/audio requests, clipping FFMS2-decoded preview windows in a reusable core helper before frame selection/PCM queueing, hard-resetting preview decode/cache/audio state on trim edits, and adding focused preview trim regression coverage at the media-preview helper layer.
  * Completed: adjusted the desktop preview follow-up so manual seek, frame-step, playback, and stop/reset behavior use the full source duration again instead of clamping to `IN/OUT`, letting users move back before a chosen trim start or past a chosen trim end while keeping the trim markers, jump buttons, and encode/export trim behavior unchanged.
  * Completed: tuned the desktop preview follow-up for lower perceived seek/play latency by enlarging paused interactive cache windows, shrinking synchronous playback refill windows, allowing adjacent manual forward requests to reuse sequential preview decode, and reducing preview-audio preroll/buffer targets so the first visible/audible response lands sooner.
  * Completed: added a timestamp-badge preview jump dialog that opens on double-click or right-click, accepts direct timecode entry plus a separate 0-based frame index field, and reuses the existing selected-job preview navigation path instead of introducing a second preview state machine.
  * Completed: hardened the preview stability follow-up by keeping seek-spam refills on explicit FFMS2 seek/decode instead of paused sequential session advance, deferring preview audio output until the first playback frame is presented while still prebuffering audio, and adding focused preview-session seek-churn coverage.
  * Completed: temporarily removed the FFmpeg `actions/cache` restore/save flow from both Windows GitHub Actions jobs, so CI now rebuilds that pinned dependency every run, and left an explicit in-workflow reminder to consider re-adding the cache on or after May 1, 2026.
  * Completed: wired the existing frame-driven streaming encode percentage into the desktop shell so encoding Status text now shows a live one-decimal percent and the existing rotating toolbar busy ring reuses that same progress value for arc length, with no broader UI redesign.
  * Completed: refined that busy ring follow-up by keeping the existing purple track, switching the progress arc to the app's gold accent, thickening the stroke without changing toolbar button size, and aligning default video CRF/preset values to `22` / `fast` across core and desktop-shell entry points.
  * Completed: hardened the libassmod RGBA subtitle adapter so transient empty bitmaps are skipped as harmless no-op output with diagnostics-only logging in both render and compose paths, while positive-size stride/null-buffer/row-overflow faults remain fatal and covered by focused render+encode regressions.
  ...
  * Completed: investigated the BorderStyle=4 direct-RGBA path without assuming RGBA output drops BS4, added a libassmod-only `ass_render_frame_rgba()` reproducer, logged raw `ASS_ImageRGBA` nodes before utsure validation, logged validation accept/reject decisions, accepted valid large clipped BS4 background tiles, and switched utsure's normal render path to upstream libassmod `ass_render_frame_auto()` behavior instead of carrying a downstream libassmod patch.
  * Completed: traced the real Kanteishi episode 06 failure to an always-active gradient watermark that keeps libassmod in RGBA output mode at the `0:20:04.76` BorderStyle=4 box, restored the CI/default libassmod pin to the Aegisub-tested `88a338192faf50505eb4cedfe7d1320265f1081f`, and moved the fix to utsure's trust boundary by accepting positive-size libassmod image nodes at collection time and defaulting burn-in composition to direct libassmod RGBA tile blending.
  * Completed: hardened the app-side direct RGBA compositor so fully off-frame subtitle tiles are clipped away before pointer/stride checks, and copied-mode RGBA isolation avoids undefined packed-stride arithmetic while keeping libassmod node collection trust-based.
  * Completed: fixed the FontCollector failure diagnostics follow-up by preserving failed tool logs under the OS temp diagnostics area, capturing stdout/stderr when the tool exits before creating its own log, and including a bounded excerpt in the blocking task-log hint so users can inspect exit-code failures after preflight cleanup without racing temporary files.
  * Completed: fixed the desktop queue-start responsiveness issue while keeping FontCollector as the primary ASS font provider: queue preflight now runs on the Qt thread pool instead of the main window thread, worker-side encode logs when FontCollector font preparation begins, and successful FontCollector preparation is cached by subtitle file state plus tool path to avoid duplicate collection during the same app session.
  * Completed: relaxed the streaming main-segment decoded SAR guard so real-world sources whose decoded frame SAR metadata disagrees with inspection can continue; the pipeline still requires the decoded main raster to match and still stamps output frames with the inspected main output SAR before subtitle composition/encoding.
  * Completed: added a narrow runtime-anomaly hardening pass: recoverable audio and intro/outro normalization now reach the job log as warnings, clipped/off-frame subtitle tiles warn once and continue safely, unsafe subtitle buffers still fail, failed/canceled streaming jobs remove any opened partial output deterministically, and the Qt worker converts unexpected core failures into structured job errors.
...
Scope:
  ...
  * Accept exact-size thumbnail images and thumbnail images whose aspect ratio matches the final output within encoder-safe resize rounding tolerance and can be normalized to the final output dimensions.
...
Current slice status:
  * Completed: added core thumbnail asset resolution for exact-size or resize-tolerant normalizable `thumbnail.*` images, matching same-stem ASS overlays, and `utsure_data` ASS text extraction/replacement while preserving override tags.
  ...
  * Completed: added focused CI tests for ASS parsing/replacement, asset resolution including exact 320x180, 320x180 -> 160x90, 1920x1080 -> 960x540, and encoder-safe rounded 1920x1080 -> 854x480 resize normalization, preflight thumbnail validation, and encode-job output frame-count behavior.

Validation:
  * Added focused core tests for asset discovery, resize-tolerant thumbnail normalization including the 1920x1080 -> 960x540 and 1920x1080 -> 854x480 user cases, 4:3 mismatch rejection, `utsure_data` ASS text parsing/replacement, preflight validation, and pre-roll output frame-count behavior.
...
Completed:
  ...
  * CRC32 fix-only polish: final logs/details now use shared UTF-8 path formatting for non-ASCII Windows paths, CRC calculation logs a single start message, collision handling uses a deterministic Copy fallback, and skipped CRC rename warnings remain visible in the completed job details.
...
Investigation:
  * Option A, strict segmented intermediate recombine, is the clean long-term model: render thumbnail/pre-roll, intro, main, and outro to identical temporary normalized intermediates, then concatenate and encode/mux the final output.
  * Option A is not a narrow urgent fix in the current architecture. The active streamer owns decode, normalization, subtitle composition, audio fallback/copy decisions, muxing, progress, partial-output cleanup, and thumbnail overlay preparation in one pass. Splitting that now would duplicate reliability policy and risk regressions in temp cleanup, selected audio, and non-ASCII path handling.
  * Option B, segment-local render clocks inside the current streaming pipeline, is the smallest reliable path for this milestone. It keeps the existing muxer and cleanup policy while enforcing that main subtitles are only rendered on main-segment frames with main-video-relative timestamps.
...
Future intermediate recombine plan:
  ...
  * Encode each segment to an intermediate with identical resolution, pixel format, frame rate/time base, sample aspect ratio, audio sample rate, channel layout, and audio codec/container settings.
  * Generate compatible silence for thumbnail/pre-roll and any segment without audio, and normalize intro/outro audio without changing the selected main audio track.
...
Completed:
  ...
  * Follow-up made `start_job()` acceptance depend on a running worker thread and a successfully queued `invokeMethod`, with state rollback on queue failure; CI now requires ffmpeg for the real app lifecycle smoke while local config keeps fake lifecycle coverage.
...
Implemented first step:
  ...
  * Updated the Windows MSYS2 workflow to run normal CI with `UTSURE_CTEST_ARGS="-LE slow"`, which skips running slow tests but still builds the current test target set.
  * Added a CI label/listing log step so the active CTest labels and filtered test inventory are visible in Actions.
...
Next cleanup steps:
  ...
  * Split build targets for optional/nightly media coverage if CI build time remains painful; the current `-LE slow` slice only filters test execution.
...
Implemented first step:
  ...
  * Follow-up made the real parallel cancel-one smoke request cancellation directly from the first `encoding_output` progress callback with a one-shot atomic guard, removing the remaining polling race.
